...
**Документация для разработчиков (модули, архитектура):** [ARCHITECTURE.md](wiki/ARCHITECTURE.md) · [EXTENDING.md](wiki/EXTENDING.md) · [TOOLS.md](wiki/TOOLS.md) · [COMPACT_TOOLS.md](wiki/COMPACT_TOOLS.md) · [wiki/README.md](wiki/README.md)
...
Сессии сохраняются автоматически в **`.tca/checkpoints.sqlite`** в рабочей директории проекта.
...
├── requirements.txt
├── wiki/                       # ARCHITECTURE.md, EXTENDING.md, TOOLS.md
│
...
│   ├── tools/                  # @tool + compact_tools.py (мульти-тулы для LLM)
...
|--------|------------------------|
| Файлы | `read_file`, `edit_file`, `write_file`, `replace_file_lines`, `insert_file_lines`, `search_in_files`, **`code_file_tool`** |
...
Защита `run_command`: блокировка опасных команд, дедупликация повторов. Снимки версий — перед правками файлов.
...
|---|---|---|
| `.tca/checkpoints.sqlite` | SQLite | Сессии (messages JSON) |
